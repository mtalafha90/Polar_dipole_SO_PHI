...
At CR\,2264 (24 PHI cases, 27~October--3~November 2022) Solar Orbiter led Earth in $B_0$ by only $\approx 2.5^{\circ}$, so the window mostly tests the machinery rather than a large polar gain. The two vantages agree on $g_{10}$ to $\approx 0.05$\,G on their common support (zero-fill mode: $-0.18$\,G for PHI versus $-0.23$\,G for HMI), and the maps correlate at $r = 0.89$ (Fig.~\ref{fig:maps2264}), establishing consistency. Against the standard polar-filled HMI synoptic chart ($g_{10,\rm ref} = +0.65$\,G) the PHI-informed product (project mode) agrees to $+0.04$\,G (6\%), while the same-pipeline Earth-view product misses by $\approx 5\times$ more, with the accuracy ordering following the polar-cap fill.
