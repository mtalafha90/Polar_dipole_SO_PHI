...
{PHI's $\ge 60^{\circ}$ polar-cap coverage overtakes HMI by up to $16\times$ (51\% versus 3\% in April 2025), switching on and off with $|B_0|$ as predicted; because the advantage is largest exactly when the Solar Orbiter--Earth separation is largest, a per-pixel map merge is invalid at the decisive epochs. At the campaign's apex, PHI recovers the correct, reversing cycle-25 polar-field polarity that HMI's grazing-angle extrapolation gets wrong, matching an independent reference chart. Injecting PHI's north-cap field into the SFT initial condition where HMI is blind shifts the first predicted polar reversal by $-1.68$\,yr; its co-observed south-cap field changes the timing by only $-0.004$\,yr.}
